...
export function calculateProtoBatch(
    ns: NS,
    target: string,
    ...
    ns.print(
        `calculated batch for ${target} (%$: ${relativeMoneyToSteal}, ms: ${spacerMs}) with ${tasks.length} tasks:`,
    );
...
export function createBatchFromProtoBatch(
    protoBatch: ProtoBatch,
